		...
		onRowClicked : function(node) {
			...
			$.ajax({
				...
				success : function(data) {
					...
					selectLicenseBean = $.extend(true, {}, data.emptyLicenseInfoBean); // 변경된 내용이 들어갈 공간에 딥카피
					
					showDetailInfo();
					showWorkInfoListGrid(data.emptyWorkInfoBean);
					showLicenseInfoListGrid(data.emptyLicenseInfoBean);
					showFamilyInfoListGrid(data.emptyFamilyInfoBean);
				}
			});
		}
	}
...
function showWorkInfoListGrid(showWorkInfoListGrid) {
	var columnDefs = [ 
...
function showWorkInfoListGrid(emptyWorkInfoBean) {
	var columnDefs = [ 
		{headerName : "사원코드", field : "empCode",},
		{headerName : "적용일", field : "workInfoDays", editable : true, 
			cellRenderer: function(params) {
		...
		{headerName : "입사일", field : "hiredate", editable : true,  hide : true,
			cellRenderer: function(params) {
	    		return  '<i class="fa fa-calendar-o" aria-hidden="true"></i>'+params.value;
	  		},
	  		//cellEditor: DateEditor  		
		},
		{headerName : "퇴사일", field : "retireDate", editable : true,  hide : true,
			cellRenderer: function(params) {
    			return  '<i class="fa fa-calendar-o" aria-hidden="true"></i>'+params.value;
  			},
  			//cellEditor: DateEditor
...
//가족정보 그리드 호출
function showFamilyInfoListGrid(emptyFamilyInfoBean) {
	var columnDefs = [ 
		...
		{headerName : "생년월일",field : "birthdate",editable : true, cellEditor : 'datePicker' ,  hide : true},
		{headerName : "동거여부",field : "liveTogether"},
		{headerName : "상태",field : "status" , hide : true}
	];
	familygridOptions = {
		columnDefs : columnDefs,
		rowData : emptyFamilyInfoBean,
		defaultColDef : {
			editable : false,
			width : 100
		},
...
//자격증정보 그리드 호출
function showLicenseInfoListGrid(showLicenseInfoListGrid) {
	var columnDefs = [ 
		{headerName : "사원코드",field : "empCode" ,editable : false},
		{headerName : "일련번호",field : "licenseCode",editable : true},
		{headerName : "자격명",field : "licenseName",editable : true},
		{headerName : "취득일",field : "getDate",cellEditor : 'datePicker' ,hide : true},
		{headerName : "만료일",field : "expireDate",cellEditor : 'datePicker' ,hide : true},
	...
	licensegridOptions = {
		columnDefs : columnDefs,
		rowData : showLicenseInfoListGrid,
		defaultColDef : {
			editable : false,
			width : 100
		},
